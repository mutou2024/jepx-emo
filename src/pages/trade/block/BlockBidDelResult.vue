<template>
  <div class="block-bid-del-result" style="padding: 20px; background-color: #f5f7fa; min-height: 100vh;">
    <el-page-header 
      :content="isBatch ? '批量区块入札删除结果' : '单条区块入札删除结果'"
      @back="$router.push('/block/bid/delete')"
    ...
    <el-card shadow="hover" style="max-width: 800px;">
      ...
      <div v-else style="text-align: center; margin: 30px 0;">
        ...
        <h2 style="color: #f56c6c; margin: 20px 0;">{{ delResult.msg || '区块入札删除失败' }}</h2>
      </div>
      ...
      <el-descriptions :column="2" border style="margin: 20px 0;">
        ...
        <el-descriptions-item label="删除的入札编号" :span="2">
          <div v-for="(id, index) in delResult.bidIds" :key="index" style="line-height: 2;">
            {{ index + 1 }}. {{ id }}
          </div>
        </el-descriptions-item>
      ...
      <div style="text-align: right; margin-top: 30px;">
        <el-button @click="$router.push('/block/bid/delete')">继续删除</el-button>
        <el-button type="info" @click="$router.push('/block/bid/query')">查看区块入札列表</el-button>
